...
For every path path $p \in I$, $d_p \leq \ell_p - 1$.
\end{lemma}
\begin{proof}
...
The \emph{length} of a path is the number of vertices it contains (including its endpoints).
...
\end{lemma}
\begin{proof}
Every path in $I^*$ has length of at least 3, 
since its end points from a disconnected pair. 
